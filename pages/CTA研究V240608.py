...
环境变量说明：
1. czsc_max_bi_num: int, 默认为 20，最大允许的笔的数量
2. CACHE_PATH，缓存路径，用于存储回测结果等数据
"""
# import sys
...
# from czsc.connectors.cooperation import get_symbols, get_raw_bars
...
base_path = Path(os.getenv("CACHE_PATH", ".cache")) / "CTA事件研究"
...
def show_signal_validation():
    ...
    czsc.show_event_return(dfs, col, sub_title="信号收益特征", max_overlap=5)
    dfs["segment"] = dfs["dt"].apply(lambda x: x.strftime("%Y年"))
    czsc.show_psi(dfs.copy(), col, "segment")
...
def show_signal_functions():
    ...
    if selected == "信号解析":
        ...
        with st.expander("查看信号代码", expanded=False):
            codes = inspect.getsource(czsc.import_by_name(res["name"]))
            st.code(codes, language="python")
        ...
        with st.expander("查看信号代码", expanded=False):
            codes = inspect.getsource(czsc.import_by_name(eval(conf)["name"]))
            st.code(codes, language="python")
...
def init_trader(files, symbol, bar_sdt, sdt, edt):
    ...
    tactic: CzscStrategyBase = JsonStreamStrategy(symbol=symbol, json_strategies=json_strategies)
...
@czsc.disk_cache(path=base_path, ttl=3600 * 24)
def get_positions_detail(files_traders, fee):
    """获取持仓策略详情"""
    pos_names = [x.name for x in czsc.dill_load(files_traders[0]).positions]
    details = {}
    for pos_name in pos_names:
        dfh, dfp = read_holds_and_pairs(files_traders, pos_name, fee=fee)
        dfr = get_daily_nv(dfh)
        details[pos_name] = {"pairs": dfp, "holds": dfh, "daily": dfr}
    return details


# @st.experimental_fragment
def show_backtest_results(file_traders, fee=2):
    details = get_positions_detail(file_traders, fee=fee)
    pos_name = st.selectbox("选择持仓策略", list(details.keys()), index=0)
    dfp = details[pos_name]["pairs"]
    dfr = details[pos_name]["daily"]
    # st.dataframe(dfp, use_container_width=True)

    show_pos_detail(file_traders[0], pos_name)

    st.subheader("一、单笔收益评价", divider="rainbow")
    # 绘制事件序列分布，横向柱状图，x轴为事件序列，y轴为交易次数，text 为交易次数
    dfe = dfp.groupby("事件序列")["标的代码"].count().reset_index().sort_values("标的代码", ascending=False)
    fig = px.bar(
        dfe,
        x="标的代码",
        y="事件序列",
    ...
    st.divider()
    st.subheader("二、品种费后收益详情", divider="rainbow")
    st.write(f"单边手续费：{fee}BP")
    dfr["edge_post_fee"] = dfr["edge_post_fee"] / 10000
    dfr["edge_pre_fee"] = dfr["edge_pre_fee"] / 10000

    with st.expander("查看品种日收益详细数据", expanded=False):
        st.dataframe(dfr, use_container_width=True)

    dfs = pd.pivot_table(dfr, index="date", columns="symbol", values="edge_post_fee", aggfunc="sum")
    dfs.index = pd.to_datetime(dfs.index)
    czsc.show_daily_return(dfs, stat_hold_days=False)

    st.subheader("三、品种等权收益曲线", divider="rainbow")
    dfd = dfr.copy().groupby("date").agg({"edge_pre_fee": "mean", "edge_post_fee": "mean"})
...
def symbol_backtest(strategies, symbol, bar_sdt, sdt, edt, results_path):
    ...
    try:
        ...
        bars = get_raw_bars(symbol, tactic.base_freq, sdt=bar_sdt, edt=edt)
        if len(bars) < 300:
            logger.warning(f"{symbol} K线数量不足，无法回测")
            return
...
def get_results_list():
    # 找出 base_path 下所有的回测结果 / 目录
    results = []
    for x in base_path.rglob("*"):
        if x.is_dir() and x.name not in [
            "__generate_signals",
            "read_holds_and_pairs",
            "get_daily_nv",
            "get_positions_detail",
        ]:
            mtime = pd.to_datetime(x.stat().st_mtime, unit="s", utc=True).tz_convert("Asia/Shanghai").tz_localize(None)
            results.append((x, mtime))
    results = sorted(results, key=lambda y: y[1], reverse=True)
    df = pd.DataFrame(results, columns=["回测结果", "更新时间"])
    return df


def backtest():
    with st.expander("回测参数设置", expanded=False):
        with st.form(key="my_form_czsc"):
            ...
            sdt = col3.date_input(label="回测开始日期", value=pd.to_datetime("2019-01-01"))
            edt = col4.date_input(
                label="回测结束日期",
                value=pd.to_datetime("2021-01-01"),
    ...
    if submit_button:
        ...
        results_path = base_path / f"{sdt}_{edt}_{hash_code}" / group
...
def show_backtest_history():
    all_results = get_results_list()
    if all_results.empty:
        st.warning(f"{base_path} 下没有任何回测结果")
    st.dataframe(all_results, use_container_width=True)

    c1, c2 = st.columns([3, 1])
    results_path = c1.selectbox("选择回测结果", all_results["回测结果"], index=0)
    fee = c2.number_input("单边费率（BP）", value=2, min_value=0, max_value=100)
    file_traders = list(results_path.rglob("*.trader"))
    if not file_traders:
    ...
    with tabs[0]:
        show_backtest_results(file_traders, fee=fee)

    with tabs[1]:
        candidates = [Path(x).stem for x in file_traders]
        with st.form(key="my_form_czsc_2"):
            # col1, col2 = st.columns([1, 3])
            sel_symbols = st.multiselect("选择品种", candidates, default=candidates[:3])
        ...
        show_backtest_results(sel_files, fee=fee)
...
def main():
    ...
    if selected == "策略回测":
        ...
        if hasattr(st.session_state, "files"):
            backtest()
